...
try:
	...
	for i in resultados:
		...
		if db.Viaje.find({"Consecutivo":i[4]}).count() != 1:
		   	client = pymongo.MongoClient()		#Hacer la coneccion con MongoDB.
			db = client ['CheckTrip'] 			#Conectar con la base de datos.
			NReg = {
				"MecanismoOrigen" : i[0],
			...
			client = pymongo.MongoClient()		#Hacer la coneccion con MongoDB.
			db = client ['CheckTrip'] 	
			post_id = db.Viaje.update({"Consecutivo":i[4]},{"$set":{"FechaInicio" : i[37],"FechaFin" : i[38], "Hospedaje.FechaEntrada" : i[43],"Hospedaje.FechaSalida" : i[44], "DatosEvento.FechaInicioPart" :i[23],"DatosEvento.FechaFinPart" :i[24]}})
		print post_id
except:
