					...
					db.viaje.find({Funcionario_id: req.params.id2}).count(function (err, count2){
						...
						db.viaje.find({Funcionario_id: req.params.id2},"GastoPasaje CostoHospedaje GastosViaticos Aclaraciones TarifaDiaria Moneda FechaFin FechaInicio Destino Funcionario_id").populate('Funcionario_id').exec(function (err, datos2){  	
							...
							var total2 = toatlhospedaje2 + totalviaticos2 + totalpasajes2
								...
								var grafica1 = "{ chart: { plotBackgroundColor: null, plotBorderWidth: 0, plotShadow: false }, title: { text: 'Total de Gastos $ "+(total1+total2)+"', align: 'center', verticalAlign: 'top', y: 50 },subtitle: {text: 'Fuente IFAI'}, tooltip: { pointFormat: '{series.name}:{point.percentage:.1f}%' }, plotOptions: { pie: { dataLabels: { enabled: true, distance: -50, style: { color: 'black', textShadow: '0px 1px 2px black' } }, startAngle: -90, endAngle: 90,center: ['50%', '75%']}},series: [{type: 'pie',name: 'Porcentaje',innerSize: '50%',data: [{name: '"+datos1[0].Funcionario_id.Nombre.Nombres+" $"+total1+"',y: "+total1+",color: '#FFD54F'},{name: '"+datos2[0].Funcionario_id.Nombre.Nombres+" $"+total2+"',y: "+total2+"}]}]}"
								var grafica12 = "{ chart: { plotBackgroundColor: null, plotBorderWidth: 0, plotShadow: false }, title: { text: 'Total de Viajes "+(count1+count2)+"', align: 'center', verticalAlign: 'top', y: 50 },subtitle: {text: 'Fuente IFAI'}, tooltip: { pointFormat: '{series.name}:{point.percentage:.1f}%' }, plotOptions: { pie: { dataLabels: { enabled: true, distance: -50, style: { color: 'black', textShadow: '0px 1px 2px black' } }, startAngle: -90, endAngle: 90,center: ['50%', '75%']}},series: [{type: 'pie',name: 'Viajes',innerSize: '50%',data: [{name: '"+datos1[0].Funcionario_id.Nombre.Nombres+" "+count1+"',y: "+count1+",color: '#FFD54F'},{name: '"+datos2[0].Funcionario_id.Nombre.Nombres+" "+count2+"',y: "+count2+"}]}]}"
								var grafica2 = "{chart: {type: 'bar'},title: {text: 'Gastos por seccion'},subtitle: {text: 'Fuente IFAI'},xAxis: {categories: ['"+datos1[0].Funcionario_id.Nombre.Nombres+"', '"+datos2[0].Funcionario_id.Nombre.Nombres+"'],title: {text: null}},yAxis: {min: 0,title: {text: 'Pesos MXP',align: 'high'},labels: {overflow: 'justify'}},tooltip: {valueSuffix: ' Pesos'},plotOptions: {bar: {dataLabels: {enabled: true}}},legend: {layout: 'vertical',align: 'right',verticalAlign: 'top',x: -40,y: 10,floating: true,borderWidth: 1,backgroundColor:  '#FFFFFF',shadow: true},credits: {enabled: false},series: [{name: 'Gastos en Hospedaje',color: '#03a9f4',data: ["+toatlhospedaje1+", "+toatlhospedaje2+"]}, {name: 'Gastos en Viaticos',color:'rgba(0,0,0,0.6)',data: ["+totalviaticos1+", "+totalviaticos2+"]}, {name: 'Gastos en Pasaje',color: 'orange',data: ["+totalpasajes1+", "+totalpasajes2+"]}]}"
						  		var link = "http://localhost:3000/compare/" + req.params.id +"/"+req.params.id2 + "/"+req.params.tipo
					...
					db.viaje.find({_id: req.params.id2}).count(function (err, count2){
						...
						db.viaje.find({_id: req.params.id2},"ViaticosComp ViaticosNoComp ViaticosDevueltos Tema MecanismoOrigen TarifaDiaria Consecutivo GastoPasaje CostoHospedaje GastosViaticos Aclaraciones  Moneda FechaFin FechaInicio Destino Funcionario_id").populate('Funcionario_id').exec(function (err, datos2){  	
							...
							var totales ={
								...
								var grafica1 = "{ chart: { plotBackgroundColor: null, plotBorderWidth: 0, plotShadow: false }, title: { text: 'Total de Gastos $ "+(total1+total2)+"', align: 'center', verticalAlign: 'top', y: 50 },subtitle: {text: 'Fuente IFAI'}, tooltip: { pointFormat: '{series.name}:{point.percentage:.1f}%' }, plotOptions: { pie: { dataLabels: { enabled: true, distance: -50, style: { color: 'black', textShadow: '0px 1px 2px black' } }, startAngle: -90, endAngle: 90,center: ['50%', '75%']}},series: [{type: 'pie',name: '$',innerSize: '50%',data: [{name: '"+datos1[0].Consecutivo+" $"+total1+"',y: "+total1+",color: '#FFD54F'},{name: '"+datos2[0].Consecutivo+" $"+total2+"',y: "+total2+"}]}]}"
								var grafica12 = "{ chart: { plotBackgroundColor: null, plotBorderWidth: 0, plotShadow: false }, title: { text: 'Total Aclaraciones Solicitadas', align: 'center', verticalAlign: 'top', y: 50 },subtitle: {text: 'Fuente IFAI'}, tooltip: { pointFormat: '{series.name}:{point.percentage:.1f}%' }, plotOptions: { pie: { dataLabels: { enabled: true, distance: -50, style: { color: 'black', textShadow: '0px 1px 2px black' } }, startAngle: -90, endAngle: 90,center: ['50%', '75%']}},series: [{type: 'pie',name: '$',innerSize: '50%',data: [{name: '"+datos1[0].Consecutivo+" "+datos1[0].Aclaraciones+"',y: "+datos1[0].Aclaraciones+",color: '#FFD54F'},{name: '"+datos2[0].Consecutivo+" "+datos2[0].Aclaraciones+"',y: "+datos2[0].Aclaraciones+"}]}]}"
								...
								var link = "http://localhost:3000/compare/" + req.params.id +"/"+req.params.id2 + "/"+req.params.tipo
								res.render('compare',{count1:count1, count2:count2, datos1:datos1,datos2:datos2, totales:totales,grafica1:grafica1,grafica2:grafica2,grafica12:grafica12, tipo:0,link:link});
						});
					});
			  });
		});
